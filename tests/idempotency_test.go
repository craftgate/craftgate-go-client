...
func Test_Idempotency_PathOnlyRequest_SendsKeyAndNoQueryOrBody(t *testing.T) {
	...
	req, err := idempotencyClient.NewRequestWithIdempotencyKey(context.Background(), http.MethodDelete,
		"/payment/v1/checkout-payments/token-1", nil, request.IdempotencyKey)
	if err != nil {
		t.Fatalf("Error %s", err)
	}
...
func Test_Idempotency_PathOnlySignatureIsUnchangedByKey(t *testing.T) {
	...
	a, err := idempotencyClient.NewRequestWithIdempotencyKey(context.Background(), http.MethodDelete, path, nil,
		"idempotency-key-1")
	if err != nil {
		t.Fatalf("Error %s", err)
	}
	b, err := idempotencyClient.NewRequestWithIdempotencyKey(context.Background(), http.MethodDelete, path, nil, "")
	if err != nil {
		t.Fatalf("Error %s", err)
	}
...
func Test_Idempotency_KeyOfReadsEmbeddedField(t *testing.T) {
	request := adapter.DeleteProductRequest{Id: 42}
	request.IdempotencyKey = "idempotency-key-1"

	if got := adapter.IdempotencyKeyOf(request); got != "idempotency-key-1" {
		t.Errorf("expected idempotency-key-1, got %q", got)
	}
	if got := adapter.IdempotencyKeyOf(&request); got != "idempotency-key-1" {
		t.Errorf("expected idempotency-key-1 for pointer, got %q", got)
	}
	if got := adapter.IdempotencyKeyOf(adapter.DeleteProductRequest{Id: 42}); got != "" {
		t.Errorf("expected empty key, got %q", got)
	}
	if got := adapter.IdempotencyKeyOf(nil); got != "" {
		t.Errorf("expected empty key for nil, got %q", got)
	}
	if got := adapter.IdempotencyKeyOf("not-a-struct"); got != "" {
